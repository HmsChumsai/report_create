...
--common.CheckValidTimeRange(entryFrom, entryTo)
  ...
  local entryFrom = tradingCalendar:addActiveDays( tim.Date.current(),-1, isNONE ):toString("%Y-%m-%dT16:00")
  ...
  for _,no in pairs( orders ) do
    ...
    if( order:getAfterCloseOrder() == "Pending" or  order:getAfterCloseOrder() == "Triggered")then
    print("===============================================================================================")
    ...
    table.insert (depositItem,{'Cancel_ID',op:getEntryUserId()})
    ...
    table.insert (depositItem,{'Trade__date',order:getEntryTime():getDateInUtcOffset(common.offsetTimeZoneSecs):toString("%d/%m/%Y")})
