  ...
  for _,deposit in pairs( deposits ) do
    ...
    if( profile ~= nil) then
        ...
        if( string.find("F=;O=;A=;N=;G=;E=;X=;", profilePreFix) ~= nil) then
          ...
          if (OrderVolumerisk:getCallForce() == "Call") then
          local depositItem = {}
          ...
          table.insert(depositItem,{'TRADENAME',Deposit:getName()})
          depositId = Deposit:getNumber()
          --print("- DEPOSIT ID: "..depositId)
          table.insert (depositItem, {'DEPOSITID',depositId})
          
          --print("-* Customer Type = "..ordVolLim:getUniqueRisk():getCallForce())
          table.insert (depositItem, {'TYPE',ordVolLim:getUniqueRisk():getCallForce()})
          
          --print("-* Cash Balance = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getCashBalance()))
          table.insert (depositItem, {'CASH',easygetter.EvenAmountToDouble(OrderVolumerisk:getCashBalance())})
          
          --print("-* Floating P&L or Unrealized = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getTotalFutureUPLGross()))
          table.insert (depositItem, {'FLOATING',easygetter.EvenAmountToDouble(OrderVolumerisk:getTotalFutureUPLGross())})
          
          --print("-* EE Balance = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getExcessEquityBalance()))
          table.insert (depositItem, {'EEBALANCE',easygetter.EvenAmountToDouble(OrderVolumerisk:getExcessEquityBalance())})
          
          --print("-* Margin Balance = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getPositionMargin()))
          table.insert (depositItem, {'MARGINBALANCE',easygetter.EvenAmountToDouble(OrderVolumerisk:getPositionMargin())})
          
          --print("-* Unrealized Margin Balance = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getPositionMargin())-easygetter.EvenAmountToDouble(OrderVolumerisk:getDailyFutureUPLGross()))
          ...
          --print("-* Equity Balance = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getEquityBalance()))
          table.insert (depositItem, {'EQUITYBALANCE',easygetter.EvenAmountToDouble(OrderVolumerisk:getEquityBalance())})
          
          --print("-* Total MM = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getMaintenanceMargin()))
          table.insert (depositItem, {'TOTALMM',easygetter.EvenAmountToDouble(OrderVolumerisk:getMaintenanceMargin())})
          
          --print("-* Call Amount = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getCallAmount()))
          table.insert (depositItem, {'CALLAMOUNT',easygetter.EvenAmountToDouble(OrderVolumerisk:getCallAmount())})
          
          --print("-* Force Amount = "..easygetter.EvenAmountToDouble(OrderVolumerisk:getForceAmount()))
          ...
          --else
          -- print("- DEPOSIT ID: ".."FAIL")
        end
        
    end
    -- table.insert (depositList, depositItem)-------* NOT WORK
   end
--table.insert (depositList, depositItem)------------* NOT WORK
